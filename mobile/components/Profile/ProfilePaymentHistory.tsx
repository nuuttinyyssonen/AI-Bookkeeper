            ...
            <View className="divide-y divide-slate-100">
                {paymentHistory.map((payment, key) => (
                    <View className="flex-row items-center justify-between py-3" key={key}>
                        <View>
                            <Text className="text-sm font-medium text-slate-950">{payment.description}</Text>
                            <Text className="text-xs text-slate-500">{payment.date}</Text>
                        </View>
                        <View className="flex-row items-center gap-3">
                            <View className="rounded-full border border-teal-200 bg-teal-50 px-2 py-0.5">
                                <Text className="text-xs font-medium text-teal-800">{payment.status}</Text>
                            </View>
                            <Text className="text-sm font-medium text-slate-950">{payment.amount} €</Text>
                        </View>
                    </View>
                ))}
            </View>
