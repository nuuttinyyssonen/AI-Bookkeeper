        ...
        <View className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <View className="flex-1">
                    ...
                    {receiptView.isEditing ? (
                        <TextInput
                            ...
                            className="h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-950"
                        />
                    ) : (
                        <Text className="text-lg font-semibold text-slate-950">{receiptView.receipt.vendor_name}</Text>
                    )}
                </View>
            </View>
                ...
                <View className="flex-1 items-end">
                    ...
                    {receiptView.isEditing ? (
                        <TextInput
                            ...
                            className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-right text-sm text-slate-950"
                        />
                    ) : (
                        <Text className="text-lg font-semibold text-slate-950">{receiptView.receipt.total_amount} €</Text>
                    )}
                </View>
            </View>
            ...
            {receiptView.isEditing ? (
                receiptView.editVats.length > 0 && (
                    <View className="mt-4 gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
                        ...
                        {receiptView.editVats.map((vat: any) => (
                            <View key={vat.id} className="gap-2">
                                <View className="flex-row items-center gap-2">
                                    <Text className="w-16 text-xs text-slate-500">Kanta %</Text>
                                    <TextInput
                                        value={vat.rate}
                                ...
                                <View className="flex-row items-center gap-2">
                                    <Text className="w-16 text-xs text-slate-500">Netto</Text>
                                    <TextInput
                                ...
                                <View className="flex-row items-center gap-2">
                                    <Text className="w-16 text-xs text-slate-500">ALV</Text>
                                    <TextInput
                                ...
                                <View className="flex-row items-center gap-2">
                                    <Text className="w-16 text-xs text-slate-500">Yhteensä</Text>
                                    <TextInput
                                        value={vat.total}
                ...
                {receiptView.isEditing ? (
                    <>
                        ...
                        <TouchableOpacity
                            ...
                            disabled={receiptView.isSaving}
                            className="h-9 items-center justify-center rounded-md border border-slate-200 bg-white px-4"
                        >
                            <Text className="text-sm font-semibold text-slate-700">Peruuta</Text>
                        </TouchableOpacity>
                    </>
                ) : (
                    <>
                        <TouchableOpacity onPress={receiptView.handleEdit} className="h-9 items-center justify-center rounded-md bg-slate-950 px-4">
                            <Text className="text-sm font-semibold text-white">Muokkaa kuittia</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={receiptView.handleDeleteReceipt} className="h-9 items-center justify-center rounded-md bg-red-600 px-4">
