...
export default function PurchasesVAT({ report }: UseReportByIdReturn) {
    return (
        <View className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 shadow-sm">
            <View className="border-b border-slate-100 dark:border-slate-800 px-5 py-4">
                <Text className="font-semibold text-slate-900 dark:text-slate-50">Ostojen ALV-erittely</Text>
                <Text className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Ostoistasi vähennyskelpoinen ALV</Text>
            </View>
            ...
            {report.vat_breakdown.purchases.map((row: VatBreakdownRow) => (
                <View key={row.rate} className="flex-row border-b border-slate-100 dark:border-slate-800 px-5 py-3">
                    ...
                    <Text className="flex-1 text-right text-sm font-medium text-slate-700 dark:text-slate-200">{row.vat_amount.toFixed(2)} €</Text>
            ...
            <View className="flex-row bg-slate-50 dark:bg-slate-900 px-5 py-3">
                ...
                <Text className="flex-1 text-right text-sm font-semibold text-slate-900 dark:text-slate-50">{parseFloat(report.purchase_net).toFixed(2)} €</Text>
                <Text className="flex-1 text-right text-sm font-semibold text-slate-700 dark:text-slate-200">{parseFloat(report.purchase_vat_amount).toFixed(2)} €</Text>
                <Text className="flex-1 text-right text-sm font-semibold text-slate-900 dark:text-slate-50">{(parseFloat(report.purchase_gross)).toFixed(2)} €</Text>
            </View>
        </View>
    );
};