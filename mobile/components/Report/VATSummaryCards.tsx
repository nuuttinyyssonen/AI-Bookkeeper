    ...
    return (
        <View className="gap-4">
            <View className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <Text className="mt-2 text-2xl font-semibold text-teal-600">{parseFloat(report.sales_vat_amount).toFixed(2)} €</Text>
                <Text className="mt-1 text-xs text-slate-500">Veroton {parseFloat(report.sales_net).toFixed(2)} €</Text>
            </View>
            <View className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <Text className="text-xs font-medium uppercase tracking-wider text-slate-400">Ostojen ALV</Text>
                <Text className="mt-2 text-2xl font-semibold text-slate-700">{parseFloat(report.purchase_vat_amount).toFixed(2)} €</Text>
                <Text className="mt-1 text-xs text-slate-500">Veroton {parseFloat(report.purchase_net).toFixed(2)} €</Text>
            </View>
