...
exports.exportReceiptsToExcel = exports.changeReceiptDeductibilityPercentage = exports.changeReceiptDeductible = exports.changeReceiptCategory = exports.updateReceipt = exports.getReceiptStatus = exports.getReceiptById = exports.getAllReceiptsByUserId = void 0;
 ...
 */
const getAllReceiptsByUserId = async (req, res, next) => {
    const user = req.user;
    // Validate and parse query params
    const queryResult = receipt_schema_1.receiptQuerySchema.safeParse(req.query);
    if (!queryResult.success) {
        return next(new error_1.ValidationError(queryResult.error.issues[0].message));
    }
    ...
    try {
        ...
        const [receipts, total, expenseTotal, incomeTotal, pending_document_count, processing_document_count] = await Promise.all([
            ...
            prisma_1.prisma.receipt.count({ where: { user_id: user.id, receipt_type: 'EXPENSE' } }),
            prisma_1.prisma.receipt.count({ where: { user_id: user.id, receipt_type: 'INCOME' } }),
            prisma_1.prisma.document.count({ where: { user_id: user.id, status: 'PENDING' } }),
            prisma_1.prisma.document.count({ where: { user_id: user.id, status: 'PROCESSING' } }),
        ]);
        ...
        next(new error_1.ServerError("Internal server error"));
    }
};
exports.getAllReceiptsByUserId = getAllReceiptsByUserId;
/**
...
const getReceiptById = async (req, res, next) => {
    ...
    try {
        const receipt = await prisma_1.prisma.receipt.findUnique({ where: { id, user_id: req.user.id }, include: { receiptVats: true } });
        if (!receipt) {
            return next(new error_1.NotFoundError("Resource not found"));
        }
...
const getReceiptStatus = async (req, res, next) => {
    ...
    const result = id_schema_1.batchIdSchema.safeParse(req.params);
    if (!result.success) {
        return next(new error_1.ValidationError(result.error.issues[0].message));
    }
    ...
    try {
        ...
        const pending_documents = await prisma_1.prisma.document.count({
            where: {
                user_id: user.id,
                status: "PENDING",
        ...
        const completed_documents = await prisma_1.prisma.document.count({
            where: {
                user_id: user.id,
                status: "COMPLETED",
                upload_batch_id: batchId
        ...
        const processing_documents = await prisma_1.prisma.document.count({
            where: {
                user_id: user.id,
                status: "PROCESSING",
                upload_batch_id: batchId
        ...
        const total = await prisma_1.prisma.document.count({
            where: {
                user_id: user.id,
                upload_batch_id: batchId
            }
        });
        const failed_documents = await prisma_1.prisma.document.count({
            where: {
                user_id: user.id,
                status: "FAILED",
                upload_batch_id: batchId
...
const updateReceipt = async (req, res, next) => {
    ...
    const bodyResult = receipt_schema_1.updateReceiptSchema.safeParse(req.body);
    if (!idResult.success) {
        return next(new error_1.ValidationError(idResult.error.issues[0].message));
    }
    if (!bodyResult.success) {
        return next(new error_1.ValidationError(bodyResult.error.issues[0].message));
    }
    ...
    try {
        await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (vats && vats.length > 0) {
                await Promise.all(vats.map(vat => tx.receiptVat.update({
 ...
 */
const changeReceiptCategory = async (req, res, next) => {
    // Getting category from req.body and id from params and validating with zod
    const cateogryResult = receipt_schema_1.categorySchema.safeParse(req.body);
    const idResult = id_schema_1.idSchema.safeParse(req.params);
    if (!cateogryResult.success) {
        return next(new error_1.ValidationError(cateogryResult.error.issues[0].message));
    }
    if (!idResult.success) {
        return next(new error_1.ValidationError(idResult.error.issues[0].message));
    }
...
exports.changeReceiptCategory = changeReceiptCategory;
/**
 ...
 * @throws {ServerError} 500 - If database update fails
 */
const changeReceiptDeductible = async (req, res, next) => {
    // Getting category from req.body and id from params and validating with zod
    const deductibleResult = receipt_schema_1.isDeductibleSchema.safeParse(req.body);
    const idResult = id_schema_1.idSchema.safeParse(req.params);
    if (!deductibleResult.success) {
        return next(new error_1.ValidationError(deductibleResult.error.issues[0].message));
    }
    if (!idResult.success) {
        return next(new error_1.ValidationError(idResult.error.issues[0].message));
    }
...
exports.changeReceiptDeductible = changeReceiptDeductible;
/**
 ...
 * @throws {ServerError} 500 - If database update fails
 */
const changeReceiptDeductibilityPercentage = async (req, res, next) => {
    // Getting category from req.body and id from params and validating with zod
    const deductibleResult = receipt_schema_1.deductibilityPercentageSchema.safeParse(req.body);
    const idResult = id_schema_1.idSchema.safeParse(req.params);
    if (!deductibleResult.success) {
        return next(new error_1.ValidationError(deductibleResult.error.issues[0].message));
    }
...
exports.changeReceiptDeductibilityPercentage = changeReceiptDeductibilityPercentage;
/**
 ...
 */
const exportReceiptsToExcel = async (req, res, next) => {
    const user = req.user;
    const queryResult = receipt_schema_1.receiptQuerySchema.safeParse(req.query);
    if (!queryResult.success) {
        return next(new error_1.ValidationError(queryResult.error.issues[0].message));
...
exports.exportReceiptsToExcel = exportReceiptsToExcel;
