...
const retryWithBackoff = async (fn, maxAttempts = 5, initialDelayMs = 1000) => {
    let lastError;
...
const worker = new bullmq_1.Worker("receiptQueue", async (job) => {
    // Extract filePath and receipt_type from job data
    const { filePath } = job.data;
    const { receipt_type } = job.data;
    // Winston logging
    logger_1.default.info({ message: "Processing receipt", filePath, jobId: job.id });
    if (!filePath) {
    ...
    try {
        fileBuffer = await retryWithBackoff(() => (0, supabase_service_1.downloadFileFromSupabase)(filePath), 5, 500);
    }
    catch (err) {
    ...
    const document = await retryWithBackoff(() => prisma_1.prisma.document.findFirst({
        where: {
            file_path: filePath
        }
    }), 5, 500);
    ...
    const { fullText } = await (0, ocr_service_1.analyzeReceipt)(fileBuffer, document?.document_type ?? undefined);
    const aiData = await (0, openai_service_1.parseReceiptData)(fullText, categoryTypes, receipt_type ?? 'EXPENSE');
    ...
    const category = (aiData.category && categoryTypes.includes(aiData.category)
        ? await prisma_1.prisma.category.findUnique({ where: { type: aiData.category } })
        : null) ?? await prisma_1.prisma.category.findUnique({
    ...
    try {
        ...
        const receipt = await prisma_1.prisma.receipt.create({ data });
...
worker.on("failed", async (job, err) => {
    ...
    if (job?.data.filePath) {
        try {
            ...
            logger_1.default.error({ message: "Failed to update document status to FAILED", error: dbErr });
        }
    }
});
