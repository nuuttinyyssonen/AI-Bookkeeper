...
async function seedDemoData() {
    ...
    await prisma_1.prisma.chatMessage.deleteMany({ where: { chatroom: { user_id: DEMO_USER_ID } } });
    await prisma_1.prisma.chatRoom.deleteMany({ where: { user_id: DEMO_USER_ID } });
    await prisma_1.prisma.receiptVat.deleteMany({ where: { receipt: { user_id: DEMO_USER_ID } } });
    await prisma_1.prisma.receipt.deleteMany({ where: { user_id: DEMO_USER_ID } });
    await prisma_1.prisma.document.deleteMany({ where: { user_id: DEMO_USER_ID } });
    await prisma_1.prisma.vatReport.deleteMany({ where: { user_id: DEMO_USER_ID } });
    await prisma_1.prisma.subscription.deleteMany({ where: { user_id: DEMO_USER_ID } });
    await prisma_1.prisma.user.deleteMany({ where: { id: DEMO_USER_ID } });
    // Create demo user
    ...
    const categoryMap = await prisma_1.prisma.category.findMany({
        where: { type: { in: categories.map(c => c.type) } }
    });
    const cat = (type) => categoryMap.find(c => c.type === type)?.id;
    // Receipts
    const receipts = [
        {
    ...
    for (const r of receipts) {
        ...
        await prisma_1.prisma.receiptVat.createMany({
            data: r.vats.map(v => ({ ...v, receipt_id: receipt.id }))
        });
    }
    // VAT Report Q1 2026 (expenses)
    await prisma_1.prisma.vatReport.create({
        data: {
            user_id: DEMO_USER_ID,
    ...
    // Chat history
    const chatRoom = await prisma_1.prisma.chatRoom.create({
        data: {
            user_id: DEMO_USER_ID,
            title: "Kulujen analysointi",
        }
    });
    await prisma_1.prisma.chatMessage.createMany({
        data: [
