...
describe('dashboard data route', () => {
    ...
    let report_id;
    let business_id;
    let dummy_document_id;
    let q2_report_id;
    let q3_report_id;
    let q4_report_id;
    let q4_refund_report_id;
    beforeAll(async () => {
    ...
    it('Sets up receipts for VAT calculation in Q2', async () => {
        ...
        const receipts = await prisma_1.prisma.receipt.findMany({ where: { document_id: dummy_document_id } });
    ...
    afterAll(async () => {
        ...
        await prisma_1.prisma.receiptVat.deleteMany({ where: { receipt: { user_id } } });
        await prisma_1.prisma.receipt.deleteMany({ where: { user_id } });
        await prisma_1.prisma.document.deleteMany({ where: { user_id } });
        await prisma_1.prisma.vatReport.deleteMany({ where: { user_id } });
        if (user?.supabase_id) {
