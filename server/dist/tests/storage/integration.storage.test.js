...
describe("Storage routes", () => {
    ...
    beforeAll(async () => {
        ...
        if (!storageResponse.body?.[0]?.id) {
            throw new Error(`beforeAll upload failed. Status: ${storageResponse.status}, Body: ${JSON.stringify(storageResponse.body)}`);
        }
    ...
    it("Sets up another user's file for authorization checks", async () => {
        const otherUser = await (0, createUser_1.default)(`integration.storage.other.test${Date.now()}@admin.com`, "1111111-90");
        other_user_id = otherUser.id;
        const otherDocument = await prisma_1.prisma.document.create({
            data: {
        ...
        const otherReceipt = await (0, createReceipt_1.default)(otherDocument.id, other_user_id);
    ...
    it("Downloads a file successfully", async () => {
        const receipt = await (0, createReceipt_1.default)(second_document_id, user_id);
        const response = await (0, supertest_1.default)(app_1.default)
    ...
    it("Fails to upload file if format is not valid", async () => {
        const response = await (0, supertest_1.default)(app_1.default)
            .post("/api/storage/")
            .set('Cookie', `token=${token}`)
            .attach("files", path_1.default.join(__dirname, "../fixtures/test2.docx"))
    ...
    afterAll(async () => {
        ...
        if (other_user_id) {
            await prisma_1.prisma.receiptVat.deleteMany({ where: { receipt: { user_id: other_user_id } } });
            await prisma_1.prisma.receipt.deleteMany({ where: { user_id: other_user_id } });
            await prisma_1.prisma.document.deleteMany({ where: { user_id: other_user_id } });
            const otherUser = await prisma_1.prisma.user.findUnique({ where: { id: other_user_id } });
            if (otherUser?.supabase_id) {
                await supabase_1.supabaseAdmin.auth.admin.deleteUser(otherUser.supabase_id);
            }
            await prisma_1.prisma.user.delete({ where: { id: other_user_id } }).catch(() => { });
        }
    }, 10000);
});
