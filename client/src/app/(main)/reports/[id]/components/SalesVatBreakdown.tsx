            ...
            <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {vatBreakdown.sales.map((row) => (
                        <tr key={row.rate}>
                            ...
                            <td className="px-5 py-3 text-right font-medium text-teal-700 dark:text-teal-200">{row.vat_amount.toFixed(2)} €</td>
                ...
                <tfoot>
                    <tr className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-semibold">
                        ...
                        <td className="px-5 py-3 text-right text-slate-900 dark:text-slate-50">{Number(report.sales_net).toFixed(2)} €</td>
                        <td className="px-5 py-3 text-right text-teal-700 dark:text-teal-200">{Number(report.sales_vat_amount).toFixed(2)} €</td>
                        <td className="px-5 py-3 text-right text-slate-900 dark:text-slate-50">{Number(report.sales_gross).toFixed(2)} €</td>
                    </tr>
                </tfoot>
            </table>
        </div>
