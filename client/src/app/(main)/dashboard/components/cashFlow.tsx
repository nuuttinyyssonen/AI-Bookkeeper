...
export default async function Cashflow({ cashflow }: Props) {
    ...
    return (
        <section className="rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-4 text-sm">
                    ...
                    <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-amber-500 dark:bg-amber-400" />{t('expenses')}</span>
                </div>
            </div>
            <div className="mt-6 overflow-x-auto">
                <div className="grid h-72 min-w-[34rem] grid-cols-6 items-end gap-3 border-b border-slate-200 dark:border-slate-700 pb-4">
                    {cashflow.map((item) => (
                        <div key={item.month} className="flex h-full flex-col justify-end gap-3">
                            <div className="flex flex-1 items-end justify-center gap-2">
                                ...
                                <div className="w-5 rounded-t-md bg-amber-500 dark:bg-amber-400" style={{ height: `${toPercent(item.expense)}%` }} />
                            </div>
                            <p className="text-center text-xs font-medium text-slate-500 dark:text-slate-400">{item.month}</p>
                        </div>
                    ))}
                </div>
            </div>
